...
MM.fs = {
    ...
    createDir: function(path, successCallback, dirEntry) {
        ...
        MM.log('FS: Creating directory ' + paths[0] + ' in ' + MM.fs.entryURL(baseRoot) + ' from ' + paths.length, 'FS');
	/*
	for (var i=0;i<paths.length;i++) {

		baseRoot.getDirectory(paths[0],{create: true});
		baseRoot += '/
	}
	*/	
	//baseRoot.getDirectory(paths[0],{create: true},function(newDirEntry) { MM.log('test:'+newDirEntry); });
	//MM.log('bip');
    ...
    */
    findFileAndReadContents: function(filename, successCallBack, errorCallBack, dirEntry){
        ...
        MM.log('Find file and read contents2 :::: ' + filename + '//' + baseRoot);
        ...
        baseRoot.getFile(
            ...
            function(fileEntry) {
                errorCallBack(1);
                 MM.log('Find file and read contentss. ' + fileEntry.file);
            }
        );
    },
