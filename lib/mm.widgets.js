...
MM.widgets = {
    ...
    renderIframeModule: function(title, path) {
        var height = $(window).height() - 200;
	var iframer = document.getElementById("page-view-iframe");
	if(iframer.contentDocument && iframer.contentDocument.body){
		var percentHeight = parseInt(((iframer.contentDocument.body.offsetHeight - 40) / iframer.contentDocument.body.offsetHeight)*100);
        } else {
		percentHeight = 100;
	}
	var iframestyle = 'border: none; width: 100%; height: '+percentHeiht+'%';
        ...
        onloadScript+= 'iframe.style.height = (iframe.contentDocument.body.offsetHeight - 40) + "px";';
        onloadScript+= 'iframe.style.width = iframe.contentDocument.body.offsetWidth + "px";';
        onloadScript+= '}}</script>';
        var content = onloadScript + '<div style="' + divstyle + '"><div style="background-color:#154284;border:0;border-radius:0;display: table; width: 100%;height: 40px;"><div id="dialog-close" style="display: table-cell; text-align: right;padding-right:10px;color:white;padding-top:10px;">X</div></div>' + iframe + '</div>';
