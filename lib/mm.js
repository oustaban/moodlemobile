...
var MM = {
    ...
    loadSite: function(siteId, force) {
        ...
        MM.moodleWSCall(
            method          = 'local_mobile_get_courses_with_role',
	...
	MM.log('getDataFromCache:'+preSets.getFromCache+'/'+preSets.saveToCache+'/'+MM.deviceConnected());
                ...
                if (typeof(data.exception) != 'undefined') {
                    ...
                    if (data.errorcode == "invalidtoken" || data.errorcode == "accessexception") {
                        ...
                        setTimeout(function(){
                            ...
                            location.href = "index.html";
                        }, 4000); // 10 seconds later - redirect.
                        return;
                    } else {
                        if (errorCallBack) {
     ...
     * @param {Object} errorCallBack Function to be called on error.
     */
    moodleDownloadFile: function(url, path, successCallBack, errorCallBack, background, progressCallBack) {
                ...
                function(fileEntry) {
                    ...
                    ft.onprogress = function(progressEvent) {
                        if (progressEvent.lengthComputable) {
                            ...
                            //MM.log("PERCENT:"+(progressEvent.loaded / progressEvent.total));
                        } else {
                          //;
                        }
                    };
     ...
     * @param {Object} options Extra options regarding the popup layout.
     */
    popMessage2: function(text, options) {
        if (typeof options == 'undefined') {
            options = {
                autoclose: 4000
            };
        ...
        MM.widgets.dialog2(text, options);
    },
    ...
    popConfirm2: function(text, callBack, cancelCallBack) {
        var options = {
            buttons: {}
        };
    ...
    externalLinkClickHandler: function(e) {
        ...
        if (MM.touchMoving) {
            ...
            if (!MM._canUseChildBrowser()) {
                MM.log('Launching childBrowser:'+link);
                link = link.replace('file:///', 'cdvfile://');
                MM.log('Launching childBrowser:'+link);
                //link = link.toURL();
                MM.log('Launching childBrowser:'+link);
                try {
                ...
                MM.log('Opening external link using navigator.app:'+link);
                navigator.app.loadUrl(link, { openExternal:true } );
                //MM._openFile(link);
                //$('.modalContent').html(MM.settings.getDeviceInfo());
                //window.open(link, '_blank','location=no','toolbar=yes','closebuttoncaption=Return');
                window.open(link, '_system');
            } else {
                MM.log('Opening external link using window.open');
                //window.open(link, '_blank');
                window.open(link, '_system','location=no','toolbar=yes','closebuttoncaption=Return');
                //window.open(link, '_self','location=no','toolbar=yes','closebuttoncaption=Return');
            }
        }
        ...
        if (MM.inNodeWK) {
            ...
            if (MM.deviceOS == 'android' && window.plugins.webintent) {
                ...
                window.plugins.webintent.startActivity(
                    iParams,
                    function() {
                        MM.log('Intent launched :' +link+','+mimetype['type']);
                    },
                    function() {
    ...
    refresh: function() {
        if (MM.deviceConnected()) {
            MM.Router.navigate("");
            MM.cache.invalidate();
	    els = MM.db.where('users', {'site': MM.config.current_site.id});
	    _.each(els, function(el) {
	    	el = el.toJSON();
		id = MM.config.current_site.id + '-' + el.userid;
	    	MM.db.remove('users', id);
	    });
